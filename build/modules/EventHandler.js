...
class EventHandler {
    constructor(comment, storage, user, input, btn, output, maxText, maxLength, emptyCommentsMessage) {
        this.MAXLENGTH = 1000;
        this.replyToCommentId = null;
        this.previousElement = null;
        this.newElement = document.createElement("span");
        this.containerText = document.querySelector(".form-text-container");
    ...
    initializeInputHandler() {
        this.input.addEventListener("keyup", () => {
            var _a;
            const textLength = ((_a = this.input.textContent) === null || _a === void 0 ? void 0 : _a.length) || 0;
    ...
    sendMessageHandler() {
        this.btn.addEventListener("click", () => {
            var _a;
            const commentText = ((_a = this.input.textContent) === null || _a === void 0 ? void 0 : _a.trim()) || "";
            ...
            if (this.replyToCommentId !== null) {
                ...
                const replyCommentHTML = this.comment.generateCommentReplyHTML(replyData, // Используем объект ответа
                parentComment, parentComment.answers.length - 1);
                const replyCommentsContainer = document.querySelector(`.main-comments[data-id="${this.replyToCommentId}"] .reply-comments__container`);
                replyCommentsContainer.insertAdjacentHTML("beforeend", replyCommentHTML);
                ...
                const commentHTML = this.comment.generateCommentHTML(commentData, this.comment.saveData.length - 1);
    ...
    updateCommentCount() {
        ...
        const totalComments = this.comment.saveData.reduce((total, comment) => { var _a; return total + 1 + (((_a = comment.answers) === null || _a === void 0 ? void 0 : _a.length) || 0); }, 0);
        if (numberComments) {
    ...
    replyHandler() {
        document.addEventListener("click", (event) => {
            ...
            if (target && target.classList.contains("reply-button")) {
                ...
                const id = (_a = target.closest(".main-comments")) === null || _a === void 0 ? void 0 : _a.getAttribute("data-id");
                // Проверяем, что id не undefined и не null
                if (id !== undefined && id !== null) {
                    this.replyToCommentId = parseInt(id, 10);
                    if (this.containerText) {
                        const { firstName, lastName } = this.comment.saveData[this.replyToCommentId];
    ...
    ratingCommentHandler() {
        document.addEventListener("click", (event) => {
            ...
            if (target.classList.contains("rating-button__negative") ||
                ...
                const isReply = target.classList.contains("rating-button__negative--reply") ||
                    target.classList.contains("rating-button__positive--reply");
                ...
                const commentElement = target.closest(`.${targetClass}`);
                if (!commentElement)
                    return; // Проверка на наличие комментария
                const commentId = commentElement.getAttribute("data-id");
                const currentScoreElement = commentElement.querySelector(`.${scoreClass}`);
                if (!currentScoreElement)
                    return; // Проверка на наличие элемента оценки
                ...
                else if (target.classList.contains("rating-button__positive") ||
                    target.classList.contains("rating-button__positive--reply")) {
                    if (oldRating === currentScore || oldRating > currentScore) {
                        currentScore += 1;
                    }
                ...
                const parentId = isReply
                    ? parseInt(((_a = commentElement
                        .closest(".main-comments")) === null || _a === void 0 ? void 0 : _a.getAttribute("data-id")) || "", 10)
                    : null;
            ...
            if (target.classList.contains("favorites-button") ||
                target.classList.contains("favorites-button-img")) {
                const commentElement = target.closest(".main-comments, .reply-comments");
                if (!commentElement)
                    return; // Проверка на наличие комментария
                const commentId = commentElement.getAttribute("data-id");
                const isReply = commentElement.classList.contains("reply-comments");
                const parentId = isReply
                    ? parseInt(((_b = commentElement
                        .closest(".main-comments")) === null || _b === void 0 ? void 0 : _b.getAttribute("data-id")) || "", 10)
                    : null;
                let comment;
                if (isReply && parentId !== null) {
                    ...
                    if (!isNaN(answerId)) {
                        ...
                        if (comment && comment.answers[answerId]) {
                            ...
                            this.updateFavoritesButton(commentElement.querySelector(".favorites-button-img"), commentElement.querySelector(".favorites-button"), comment.answers[answerId].favorites);
                        }
                    }
                }
                else {
                    ...
                    if (!isNaN(commentIdNumber)) {
                        ...
                        if (comment) {
                            ...
                            this.updateFavoritesButton(commentElement.querySelector(".favorites-button-img"), commentElement.querySelector(".favorites-button"), comment.favorites);
                        }
                    }
                }
    ...
    updateFavoritesButton(imgElement, textElement, isFavorite) {
        if (isFavorite) {
