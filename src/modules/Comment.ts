...
interface CommentData {
  ...
  favorites: boolean;
  rating: number;
  answers: AnswerData[]; // Главные комментарии могут содержать ответы
}
...
interface AnswerData {
  ...
  favorites: boolean;
  rating: number;
  // Ответы не содержат поле "answers"
}
...
class Comment {
  ...
  getDateComments(date: string): string {
  ...
  generateCommentHTML(commentData: CommentData, index: number): string {
    const ratingColor =
      commentData.rating < 0 ? "color: red;" : "color: #8ac540;";
    return `
          ...
          <div class="main-comments__buttons-container">
            ...
            <span class="favorites-button" data-old="${commentData.favorites}">
              <img src="./assets/icons/${
                commentData.favorites ? "in" : "to"
              }-favorites.svg" class="favorites-button-img" alt="${
  ...
  generateCommentReplyHTML(
    answerData: AnswerData, // Ответы используют AnswerData
    parentComment: CommentData, // Главный комментарий
    indexAnswerId: number
  ): string {
    const ratingColor =
      answerData.rating < 0 ? "color: red;" : "color: #8ac540;";
    return `
  ...
  renderComments(): void {
    ...
    this.saveData.forEach((commentData, index) => {
      ...
      if (commentData.answers.length > 0) {
        const replyCommentsContainer = this.container.querySelector(
        ...
        if (replyCommentsContainer) {
          commentData.answers.forEach((answer, answerIndex) => {
            const replyHTML = this.generateCommentReplyHTML(
              answer, // Используем AnswerData для ответов
