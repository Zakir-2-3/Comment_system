...
interface AnswerData extends Omit<CommentData, "answers"> {} // Ответы, у которых нет поля answers

class Filter {
  filterBtn: HTMLButtonElement;
  filterMenu: HTMLElement;
  filterListContainer: HTMLElement;
  answerMessageContainer: HTMLElement;
  btnSortingFavoritesComments: HTMLSpanElement;
  comment: Comment; // Экземпляр класса Comment
  btnSortingComments: HTMLImageElement;
  currentFilterIndex: number; // По умолчанию фильтр "По дате"
  ...
  initializeFilterMenu(): void {
    this.filterBtn.addEventListener("click", () => {
      ...
      this.filterBtn.setAttribute(
        "aria-expanded",
        this.filterBtn.classList.contains("filter-btn--open") ? "true" : "false"
      );
  ...
  applyFilterSelection(): void {
    const filterItems = this.filterListContainer.querySelectorAll(
      ".filter-comment__container-menu--item"
    );
    ...
    filterItems.forEach((item, index) => {
      item.addEventListener("click", () => {
        ...
        const currentActiveItem =
          this.filterListContainer.querySelector(".filter-active");
  ...
  filterComments(index: number): CommentData[] {
    let filteredComments: CommentData[] = [];

    if (index === 0) {
      filteredComments = [...this.saveData].sort((a, b) =>
        this.isReverseSorting
      ...
      filteredComments = [...this.saveData].sort((a, b) =>
        this.isReverseSorting ? a.rating - b.rating : b.rating - a.rating
      );
  ...
  resetToDefaultFilter(): void {
    ...
    const defaultFilterItem = this.filterListContainer.querySelector(
      ".filter-comment__container-menu--item:nth-child(1)"
    );
    if (defaultFilterItem) {
      ...
      checkMarkImg.src = "./assets/icons/check-mark.svg";
      checkMarkImg.alt = "check-mark";
      defaultFilterItem.insertBefore(
        checkMarkImg,
        defaultFilterItem.firstChild
      );
  ...
  updateComments(filteredComments: CommentData[]): void {
    ...
    filteredComments.forEach((commentData, index) => {
      const addCommentHTML = this.comment.generateCommentHTML(
        commentData,
        index
      );
      this.answerMessageContainer.insertAdjacentHTML(
        "beforeend",
        addCommentHTML
      );

      // Обновляем иконки избранного для каждого комментария
      const favoritesButtonImg = document.querySelector<HTMLImageElement>(`
      ...
      if (commentData.answers.length > 0) {
        const replyCommentsContainer = document.querySelector<HTMLDivElement>(`
        ...
        if (replyCommentsContainer) {
          commentData.answers.forEach((answer, answerIndex) => {
            const addReplyCommentHTML = this.comment.generateCommentReplyHTML(
              answer,
              commentData,
              answerIndex
            );
  ...
  toggleFavoritesFilter(): void {
    this.btnSortingFavoritesComments.addEventListener("click", () => {
      ...
      const favoritesAllIcon = document.querySelector(
        "#favorites-all-icon"
      ) as HTMLImageElement;

      let filteredComments: CommentData[]; // Создаем переменную для отфильтрованных комментариев
      if (this.isFavoritesFilterActive) {
        // Фильтруем комментарии и учитываем избранные ответы
        filteredComments = this.saveData.filter((comment: CommentData) => {
          ...
          return comment.answers?.some(
            (answer: AnswerData) => answer.favorites
          );
        });
